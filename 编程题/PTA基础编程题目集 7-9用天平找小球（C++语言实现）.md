...
本题（7-9 用天平找小球）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的CPP代码进行讲解。
...
3. **定义变量**：`a`、`b`、`c`均为int类型，分别存储三个球A、B、C的重量。
4. **读取输入**：`cin >> a >> b >> c;`按顺序读入三个正整数。
5. **逻辑判断（最多两次比较）：
   - 第一层：若`a == b`为真 → A与B同重 → C是异类 → 输出"C"并换行
...
### 代码执行说明
若A、B重量相等，异类球必为C；否则比较A、C，若相等则B为异类，否则A为异类。三个分支覆盖全部情况。
