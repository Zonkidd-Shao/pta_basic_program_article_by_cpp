# 7-34 通讯录的录入与显示（C++语言实现）

## 前言

本题（7-34 通讯录的录入与显示）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的CPP代码进行讲解。
...
### 算法原理说明
采用结构体数组+索引查询的方案：
1. **数据结构设计**：定义Contact结构体，包含5个字段分别存储一条记录的各项信息，其中姓名、生日、固话、手机用string存储，性别用char存储。
...
int main() {
    ...
    for (int i = 0; i < k; i++) {
        ...
        cin >> idx;
        if (idx >= 0 && idx < n) {
            cout << contacts[idx].name << " "
                 << contacts[idx].fixedPhone << " "
                 << contacts[idx].mobilePhone << " "
                 << contacts[idx].gender << " "
                 << contacts[idx].birthday << endl;
        } else {
            cout << "Not Found" << endl;
        }
    }
...
1. **头文件引入**：引入iostream和string头文件
2. **定义结构体**：struct定义Contact结构体，包含name、birthday、gender、fixedPhone、mobilePhone五个字段，其中姓名、生日、固话、手机用string存储，性别用char存储
3. **读取N**：输入通讯录记录条数n
4. **定义结构体数组**：Contact contacts[10]，最多容纳10条记录
5. **录入N条记录**：循环n次，每次用cin按顺序读取5个字段存入contacts[i]
6. **读取K**：输入查询次数k
7. **处理K次查询**：循环k次：
   - 读取查询编号idx
   - 判断idx是否在有效范围[0, n)内
   - 有效则按指定顺序输出5个字段（注意固话手机在前，性别生日在后）
   - 无效输出"Not Found"
8. **程序结束**：return 0
...
### 代码执行说明
Contact 结构体保存一条记录的五个字段；查询时检查编号范围，合法编号按题目指定顺序输出，越界编号输出 Not Found。
