# 7-11 分段计算居民水费（C++语言实现）

## 前言

本题（7-11 分段计算居民水费）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的CPP代码进行讲解。
...
1. **头文件引入与命名空间声明**
   - `#include <iostream>`：引入标准输入输出流头文件，支持cin和cout操作
   - `using namespace std;`：声明使用std命名空间，避免每次写std::前缀

2. **主函数入口**
   - `int main(void)`：程序主函数入口，void表示无参数
...
### 1. 输入与分支
程序使用 `cin` 读入用水量 `x`，使用 `if-else` 以15吨为临界点选择公式。边界 `x = 15` 必须进入第一段。

### 2. 计算与格式化
第一段计算 `4*x/3`，第二段计算 `2.5*x-17.5`。计算结果通过 `printf("%.2f")` 输出，既保留两位小数，也能正确显示整数水费。
