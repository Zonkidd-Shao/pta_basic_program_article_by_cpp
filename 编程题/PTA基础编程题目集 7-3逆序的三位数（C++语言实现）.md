...
本题（7-3 逆序的三位数）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的CPP代码进行讲解。
...
1. **引入头文件**：引入`<iostream>`用于标准输入输出。
2. **声明变量**：声明整型变量`n`存储输入的三位数。
3. **读取输入**：使用`cin`读取一个三位数。
...
8. **输出结果**：输出逆序数，整数输出自动省略前导零。

## 代码流程图

```mermaid
flowchart TD
  A["开始"] --> B["声明变量 n hundreds tens units reversed"]
  B --> C["读取三位数n"]
  C --> D["提取百位数字"]
  D --> E["提取十位数字"]
  E --> F["提取个位数字"]
...
### 代码执行说明
使用整除和取模分别提取百位、十位和个位，再将三个位按个位、十位、百位的顺序重组。结果作为整数输出，原数末尾的零会自动被省略。
