...
本题（7-5 表格输出）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的CPP代码进行讲解。
...
2. 逐行编写`cout`语句，严格复制题目给出的每一行字符串（包括空格数量）。
3. 每行末尾添加`endl`换行，确保输出格式与题目完全一致。
...
1. **引入头文件**：引入`<iostream>`用于标准输出。
2. **主函数入口**：`main(void)`表示无参数的主函数。
3. **输出顶部分隔线**：打印36个减号组成的横线，并换行。
4. **输出表头**：打印"Province"、"Area(km2)"、"Pop.(10K)"三列表头，注意空格对齐。
5. **输出表头分隔线**：再次打印36个减号横线，分隔表头与数据。
6. **输出安徽省数据**：打印"Anhui"及其对应的面积和人口数据。
7. **输出北京市数据**：打印"Beijing"及其对应的面积和人口数据。
8. **输出重庆市数据**：打印"Chongqing"及其对应的面积和人口数据。
9. **输出上海市数据**：打印"Shanghai"及其对应的面积和人口数据。
10. **输出浙江省数据**：打印"Zhejiang"及其对应的面积和人口数据。
11. **输出底部分隔线**：打印最后一条36个减号的横线，完成表格输出。
...
### 代码执行说明
程序不读取输入，使用多条输出语句按固定顺序打印分隔线、表头和五条数据。字符串中的空格与换行共同决定表格的精确对齐。
