...
本题（7-24 约分最简分式）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的CPP代码进行讲解。
...
int main() {
    ...
    char slash; // 用于接收斜杠分隔符
    // 以"分子/分母"的格式输入分数，cin 依次读取分子、斜杠、分母
...
1. **gcd函数定义**：使用辗转相除法循环计算最大公约数
   ...
   - b=0时返回a即为最大公约数
2. **主函数输入**：定义字符变量slash接收斜杠，使用cin >> numerator >> slash >> denominator依次读取分子、斜杠、分母
...
### 代码执行说明
gcd 使用辗转相除法求最大公约数，主函数读取斜杠分隔的分数后让分子、分母同时除以该公约数并输出。
