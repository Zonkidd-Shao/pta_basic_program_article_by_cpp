...
本题（7-25 念数字）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的CPP代码进行讲解。
...
int main() {
    ...
    bool first = true; // 标记是否为首个输出项，用于空格控制
    int start = 0; // 记录数字开始的位置，用于处理负号
    
    if (num[0] == '-') { // 判断是否为负数
        cout << "fu"; // 输出"fu"
        first = false; // 已输出fu，后续数字前需加空格
        start = 1; // 数字从索引1开始
    }
    
    // 遍历每个数字字符，输出对应的拼音
    for (int i = start; i < (int)num.length(); i++) {
        if (!first) cout << " "; // 非首项前加空格
        else first = false;
        cout << pinyin[num[i] - '0']; // 输出当前数字对应的拼音
    }
...
1. **输入读取**：使用cin读取数字字符串num
2. **拼音映射表**：定义pinyin数组，索引0-9对应"ling"到"jiu"
3. **负号判断**：若num[0]=='-'，输出"fu"，设置start=1；否则start=0
4. **逐位输出循环**：从start开始遍历到字符串结束
   - 非首位字符（i>start）先输出空格分隔
   - 通过num[i]-'0'将字符转为数字索引，输出对应拼音
5. **结束处理**：输出换行符，程序返回0
...
### 代码执行说明
输入按字符串保存，以便识别负号并逐位映射拼音；负数先输出 fu，数字拼音之间只输出一个空格。
