# 7-12 两个数的简单计算器（C++语言实现）

## 前言

本题（7-12 两个数的简单计算器）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的CPP代码进行讲解。
...
### 算法原理说明
使用**多分支条件判断结构**（if-else if链或switch-case）实现运算符的分类处理：
...
3. **变量定义**
   - `int a, b, result;`：定义三个整型变量
     ...
     - b：存储第二个操作数
     - result：存储运算结果
   - `char op;`：定义字符型变量op，存储运算符

4. **数据输入**
   - `cin >> a >> op >> b;`：依次读取操作数a、运算符op、操作数b
   - cin会自动跳过空格分隔符
...
5. **多分支运算处理**
   ...
   - `else if (op == '-')`：判断是否为减法运算符
     - `result = a - b;` 计算两数之差
     - 输出结果
   - `else if (op == '*')`：判断是否为乘法运算符
     - `result = a * b;` 计算两数之积
     - 输出结果
   - `else if (op == '/')`：判断是否为除法运算符
     - `result = a / b;` 计算整数除法（截断取整）
     - 输出结果
   - `else if (op == '%')`：判断是否为求余运算符
     - `result = a % b;` 计算取余结果
     - 输出结果
   - `else`：非法运算符处理
...
### 1. 读取运算表达式
程序依次读取整数 `a`、字符 `op` 和整数 `b`，空格会被输入流自动跳过，运算符由字符变量保存。

### 2. 分支执行
五个合法运算符分别对应加、减、乘、整除和取余。代码只在对应分支中计算并输出；所有未匹配的符号统一输出 `ERROR`。题目已保证除数和余数运算的分母不为零。
