...
本题（7-33 有理数加法）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的CPP代码进行讲解。
...
### 具体计算步骤
1. 按"a1/b1 a2/b2"格式读取两个分数的分子和分母（读入时用char变量吸收斜杠）
...
int main() {
    long long a1, b1, a2, b2; // 用long long防止乘积溢出
    char c;
    cin >> a1 >> c >> b1 >> a2 >> c >> b2;
    
    long long numerator = a1 * b2 + a2 * b1; // 通分后分子
    long long denominator = b1 * b2; // 通分后分母
...
1. **头文件引入**：引入iostream头文件
2. **定义gcd函数**：递归实现辗转相除法求最大公约数
3. **定义变量**：a1、b1、a2、b2分别存储两个分数的分子和分母，char变量c吸收斜杠
4. **读取输入**：使用cin读取a1、c、b1、a2、c、b2，解析带斜杠的输入
5. **计算和**：按分数加法公式计算分子和分母
6. **求GCD**：调用gcd函数求分子分母的最大公约数
7. **约分**：分子分母分别除以最大公约数
8. **输出结果**：判断分母是否为1，选择输出格式
9. **程序结束**：return 0
...
### 代码执行说明
读取两个分数并通分，得到新分子 a1*b2+a2*b1 与新分母 b1*b2；使用 gcd 约分，分母为1时省略斜杠。
