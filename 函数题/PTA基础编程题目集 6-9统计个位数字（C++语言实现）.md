...
把输入转成 `long long` 后再取绝对值，可以避免 `int` 最小值取负时溢出。对 0 单独处理，其余情况逐位取余并计数。
...
- 空间复杂度：`O(1)`，只使用当前数值和计数变量。

## 常见易错点

1. 负数只应忽略负号，统计其绝对值的数字。
2. `N = 0` 且 `D = 0` 时，数字 0 应计数 1 次。
3. 每次取出个位后必须整除 10，否则无法继续处理更高位。
4. `D` 是数字 0 到 9，不应把负号作为待统计字符。
