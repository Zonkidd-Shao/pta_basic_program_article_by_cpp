...
这里采用插入排序：将当前元素插入前面已经排好序的区间，最后直接读取中间下标 `N / 2` 的元素。

```cpp
ElementType Median(ElementType A[], int N)
{
    for (int i = 1; i < N; ++i) {
        ElementType current = A[i];
        int j = i - 1;
...
- 时间复杂度：`O(N²)`，冒泡排序最多进行 N - 1 轮比较。
- 空间复杂度：`O(1)`，排序在原数组上进行，只使用交换临时变量。

## 常见易错点

1. 必须先按从小到大排序，再取下标 `N / 2` 的元素。
2. 题目中的“第 ⌊(N+1)/2⌋ 大”与下标 `N / 2` 等价，不能误取 `A[(N-1)/2]`。
3. 交换元素时要使用与 `ElementType` 类型匹配的临时变量。
4. 排序会改变原数组内容，但不影响本题返回中位数的要求。
