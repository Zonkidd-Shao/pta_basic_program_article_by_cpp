...
首元素是最可靠的初始候选值，因此不依赖数据范围设置初值。之后只在发现更大元素时更新候选值。
...
- 时间复杂度：`O(N)`，从第二个元素开始线性比较。
- 空间复杂度：`O(1)`，只使用当前最大值、循环变量和临时变量。

## 常见易错点

1. 最大值应初始化为 `S[0]`，不能随意初始化为 0，否则全为负数时会出错。
2. 比较应从下标 1 开始，避免重复处理首元素。
3. 应返回最大元素本身，而不是返回它的下标。
