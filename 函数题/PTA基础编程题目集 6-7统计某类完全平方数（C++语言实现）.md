...
先排除负数和非完全平方数，再用计数数组统计每个数字出现的次数。某个数字计数达到 2 次即可判定成功。
...
- 时间复杂度：`O(d²)`，逐位提取后需要两两比较数字；判断平方数本身为常数次运算。
- 空间复杂度：`O(d)`，需要数组保存各位数字。

## 常见易错点

1. 必须同时满足“完全平方数”和“至少两位数字相同”两个条件。
2. `sqrt(N)` 得到整数近似值后，还要用 `x * x == N` 再次确认。
3. 比较数字时要覆盖所有不同位置的组合，不能只比较相邻数字。
4. 至少两位数字相同意味着同一数字出现两次即可，不要求连续出现。
